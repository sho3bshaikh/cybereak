...
export class TagviewComponent implements OnInit {
 ...
 public next(lastsnap){
   ...
   this.afs.collection("course").ref.where("tags","array-contains",this.tag).limit(15).startAfter(this.last).onSnapshot(
     (snap)=>{
       if(snap.empty){
        this.allloaded=true;
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      this.afs.collection("course").ref.where("tags","array-contains",this.tag).limit(15).onSnapshot((snap)=>{
      this.last=snap.docs[snap.docs.length-1]
